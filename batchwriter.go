...
// batcher.feed/batcher.finish are called; the batching logic itself lives
// entirely in the batcher.
//
// Write coalesces into pending rather than calling batcher.feed on every
// call, for the same reason as chunkWriter: BatchRoll needs many window
// positions per call to pay off. pending is flushed once it reaches
// batcher.batchSize bytes (see WithBufferSize), or on Close. This trades
// Write-to-Next latency for throughput on fragmented input. The tradeoff is
// steep at the low end: WithBufferSize(window), the technical minimum, gives
// up most of the throughput a batch size of just a few KiB would already
// recover. Prefer a small-but-not-minimal value (low single-digit KiB) over
// window if lower latency is needed; only drop all the way to window if the
...
func (w *batchWriter) Write(p []byte) (int, error) {
	...
	n := len(p)
	batchSize := max(w.batcher.batchSize, 1)
...
func (w *batchWriter) Next() bool { return w.batcher.next() == emitted }
...
func (w *batchWriter) Sums() []uint64 { return w.batcher.Sums() }
...
func (w *batchWriter) Bytes() []byte { return w.batcher.Bytes() }

// Err returns the first error encountered, if any.
func (w *batchWriter) Err() error { return w.batcher.Err() }

// Offset returns the stream position of Bytes()[0] in the current batch.
func (w *batchWriter) Offset() int { return w.batcher.Offset() }

// WindowSize returns the rolling window size passed to NewBatchWriter.
func (w *batchWriter) WindowSize() int { return w.batcher.WindowSize() }
