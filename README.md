...
Use `WithBatchSize` to control the coalescing threshold. Because `Write`
coalesces, `Next` can report no boundary yet even when the bytes written imply
one; a caller that needs a decision for exactly the bytes it has pushed can
force them through with `cw.(interface{ Flush() }).Flush()`. Only this writer
coalesces, so only this one has `Flush`; the `cdc/` writers feed every `Write`
straight through and have nothing to release.
