...
func drainWriter(t *testing.T, w *repmaxcdc.ChunkWriter, data []byte, step int) [][]byte {
	t.Helper()
	var out [][]byte
	pump := func() {
		for w.Next() {
...
func TestChunkWriterMatchesChunker(t *testing.T) {
	for _, cfg := range []struct{ min, horizon int }{
		...
		for _, data := range [][]byte{randData(300 * 1024), structData(200 * 1024), randData(40), randData(70*1024 + 3)} {
			want := collect(t, repmaxcdc.New(bytes.NewReader(data), gearhash64.New(), cfg.min, cfg.horizon))
			for _, step := range []int{1, 7, 4096, 0} {
				w := repmaxcdc.NewChunkWriter(gearhash64.New(), cfg.min, cfg.horizon)
...
func TestChunkWriterEmpty(t *testing.T) {
	w := repmaxcdc.NewChunkWriter(gearhash64.New(), 256, 512)
	if err := w.Close(); err != nil {
		t.Fatal(err)
	}
	if w.Next() {
