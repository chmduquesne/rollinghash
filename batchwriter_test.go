...
// (see the ChunkWriter/BatchWriter doc comments).
func TestBatchWriterPrePostClose(t *testing.T) {
	const window = 16
	w := rollinghash.NewBatchWriter(allHashes[0].new(), window, rollinghash.WithBufferSize(window))
...
func TestBatchWriterCoalesces(t *testing.T) {
	const window = 16
	w := rollinghash.NewBatchWriter(allHashes[0].new(), window) // default WithBufferSize: 64 KiB

	if _, err := w.Write(make([]byte, window)); err != nil {
		t.Fatal(err)
	}
	if w.Next() {
		t.Fatal("expected Next() to return false pre-Close: a single window is far below the default batch size")
	}

	w.Close()
	if !w.Next() {
		t.Fatal("expected Close to flush the coalesced window")
	}
	if len(w.Sums()) != 1 {
		t.Fatalf("got %d sums, want 1", len(w.Sums()))
	}
	if w.Next() {
...
// TestBatchWriterError verifies that Write doesn't error under normal use.
...
// TestBatchWriterErrClosed verifies that Write returns ErrClosed after
// Close, without silently dropping the write, and that Reset clears the
// closed state so the writer can be reused.
func TestBatchWriterErrClosed(t *testing.T) {
	const window = 16
	w := rollinghash.NewBatchWriter(allHashes[0].new(), window, rollinghash.WithBufferSize(window))
	...
	if _, err := w.Write(make([]byte, window)); !errors.Is(err, rollinghash.ErrClosed) {
	...
	// draining now must show exactly the one pre-Close batch, nothing more.
	got := collectBatchWriterSums(t, "errclosed", w, window)
	if len(got) != 1 {
		t.Fatalf("got %d sums after a rejected post-Close write, want 1", len(got))
	}
	...
	w.Close()
	got = collectBatchWriterSums(t, "errclosed/reset", w, window)
	if len(got) != 1 {
		t.Fatalf("got %d sums after Reset, want 1", len(got))
	}
}
...
// BenchmarkBatchWriter measures steady-state throughput via BatchWriter,
// using the same window and buffer sizes as BenchmarkBatchRoller so the two
// are directly comparable. Data is fed in writeSize pieces with Next()
// drained after every Write, matching the intended usage pattern (see
// BenchmarkChunkWriter's comment for why this matters).
	...
	for _, c := range cases {
		for _, bs := range bufSizes {
			b.Run(fmt.Sprintf("%s/buf=%dKiB", c.name, bs>>10), func(b *testing.B) {
				w := rollinghash.NewBatchWriter(c.h, window, rollinghash.WithBufferSize(bs))
